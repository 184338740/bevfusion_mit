...
class SpatialVisualizer:
    def __init__(self, config: Dict):
        ...
        self.bev_fusion_dir = self.output_dir / "bev_fusion_visualization"  # 新增BEV融合可视化目录
        
        self.output_dir.mkdir(parents=True, exist_ok=True)
        self.pointcloud_dir.mkdir(parents=True, exist_ok=True)
        self.label_dir.mkdir(parents=True, exist_ok=True)
        self.bev_fusion_dir.mkdir(parents=True, exist_ok=True)
    ...
    def create_bev_fusion_visualization(self, timestamp: str):
        """v1.0.3新增：BEV融合可视化 - 将FOV和3D Label合并显示"""
        if not MATPLOTLIB_AVAILABLE:
            print("matplotlib不可用，跳过BEV融合可视化")
            return
        ...
        output_path = self.bev_fusion_dir / f"bev_fusion_{timestamp}.jpg"
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        plt.close()
        
        # 保存配置信息
        config_path = self.bev_fusion_dir / f"bev_fusion_config_{timestamp}.txt"
        with open(config_path, 'w') as f:
    ...
    def run(self, timestamps: List[str]):
        ...
        for i, timestamp in enumerate(timestamps):
            ...
            self.create_bev_fusion_visualization(timestamp)  # 新增BEV融合可视化
