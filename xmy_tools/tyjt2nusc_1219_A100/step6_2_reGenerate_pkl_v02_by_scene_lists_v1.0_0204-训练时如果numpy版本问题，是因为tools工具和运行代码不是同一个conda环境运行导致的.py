...
class PklRegenerator:
    ...
    def __init__(self, pkl_dir: Path, scene_mapper: SceneMapper, 
    ...
    def regenerate_pkl(self, output_dir: Path):
        """重新生成pkl文件"""
        self.logger.info("开始重新生成pkl文件...")
        
        # 1. 加载原始pkl文件
        original_train_path = self.pkl_dir / "tyjt_infos_train.pkl"
        original_val_path = self.pkl_dir / "tyjt_infos_val.pkl"
        
        if not original_train_path.exists():
            raise FileNotFoundError(f"找不到训练集pkl: {original_train_path}")
        ...
        train_output_path = output_dir / "tyjt_infos_train_v02.pkl"
        val_output_path = output_dir / "tyjt_infos_val_v02.pkl"
        ...
        # 7. 生成dbinfos（可选）
        self._generate_dbinfos(train_infos, output_dir)
    ...
    def _generate_dbinfos(self, train_infos: List[Dict], output_dir: Path):
        ...
        try:
            # 从原始dbinfos复制或创建新的
            original_dbinfos_path = self.pkl_dir / "tyjt_dbinfos_train.pkl"
            new_dbinfos_path = output_dir / "tyjt_dbinfos_train_v02.pkl"
...
# ==================== 主函数 ====================
def main():
    parser = argparse.ArgumentParser(description='根据场景列表重新生成pkl文件 - V02版本')
    ...
    try:
        ...
        train_path, val_path = regenerator.regenerate_pkl(output_dir)
        ...
        logger.info(f"📄 dbinfos: {output_dir}/tyjt_dbinfos_train_v02.pkl")
        
        # 5. 创建场景列表备份
        with open(output_dir / "train_scenes_backup.txt", 'w') as f:
            for scene in sorted(train_scenes):
                f.write(f"{scene}\n")
        
        with open(output_dir / "val_scenes_backup.txt", 'w') as f:
