...
# 在step1_1_v9.2.3.py中定义新的输出结构

@dataclass
class PackageDataStructure:
    """三层数据结构：package → sub_packet → samples"""
    package_name: str
    package_type: str
    dataset_root: str
    analysis_time: str
    sub_packets: Dict[str, Dict]  # sub_packet_name -> sub_packet_info
    ...
    @classmethod
    def from_analysis_result(cls, result: 'AnalysisResult') -> 'PackageDataStructure':
        """从分析结果构建三层结构"""
        ...
        for sample in result.valid_samples:
            ...
            if sub_packet_name not in sub_packets:
                sub_packets[sub_packet_name] = {
                    "samples": [],
                    "statistics": {
                        "sample_count": 0,
                        "camera_count": 0,
            ...
            sub_packet = sub_packets[sub_packet_name]
            sub_packet["samples"].append(asdict(sample))
            sub_packet["statistics"]["sample_count"] += 1
        
        # 为每个子包计算统计信息
        for sub_packet_name, sub_packet_info in sub_packets.items():
            samples = sub_packet_info["samples"]
            if samples:
                # 按时间排序
                sorted_samples = sorted(samples, key=lambda x: x["metadata"]["timestamp_int"])
                
                # 更新时间范围
                min_ts = sorted_samples[0]["metadata"]["timestamp_int"]
                max_ts = sorted_samples[-1]["metadata"]["timestamp_int"]
                sub_packet_info["statistics"]["time_range"]["min"] = min_ts
                sub_packet_info["statistics"]["time_range"]["max"] = max_ts
                
                # 计算相机数量
                camera_counts = [s["metadata"]["camera_count"] for s in sorted_samples]
                sub_packet_info["statistics"]["camera_count"] = max(camera_counts) if camera_counts else 0
                
                # 分析时间连续性
                intervals = []
                if len(sorted_samples) > 1:
                    prev_ts = sorted_samples[0]["metadata"]["timestamp_int"]
                    for i in range(1, len(sorted_samples)):
                        curr_ts = sorted_samples[i]["metadata"]["timestamp_int"]
                        interval = (curr_ts - prev_ts) / 1e6  # 秒
                        intervals.append(interval)
                        prev_ts = curr_ts
                    
                    sub_packet_info["statistics"]["time_continuity"] = {
                        "avg_interval": np.mean(intervals) if intervals else 0,
                        "std_interval": np.std(intervals) if intervals else 0,
                        "max_interval": max(intervals) if intervals else 0,
                        "min_interval": min(intervals) if intervals else 0,
                        "is_continuous": max(intervals) <= 2.0 if intervals else True
                    }
...
class EnhancedDataAnalyzer:
    ...
    def save_enhanced_analysis_result(self, result: AnalysisResult):
        ...
        # 1. 构建三层数据结构
        package_structure = PackageDataStructure.from_analysis_result(result)
        
        # 2. 保存主要输出文件（三层结构）
        main_output_file = self.analysis_dir / f"{result.package_name}_structured_data.json"
        
        with open(main_output_file, 'w', encoding='utf-8') as f:
            json.dump(asdict(package_structure), f, indent=2, ensure_ascii=False, default=str)
        
        self.logger.info(f"💾 保存三层结构化数据: {main_output_file.name}")
        
        # 3. 保存详细统计信息
        stats_file = self.analysis_dir / f"{result.package_name}_detailed_stats.json"
        
        detailed_stats = {
            "package_info": {
                "name": result.package_name,
                "type": result.package_type,
                "total_sub_packets": len(package_structure.sub_packets),
                "total_valid_samples": result.statistics.get("total_valid_samples", 0),
                "total_invalid_samples": result.statistics.get("total_invalid_samples", 0)
            },
            "sub_packet_summary": {},
            # "sample_statistics": {
            #     "camera_distribution": self.calculate_camera_distribution(result),
            #     "time_distribution": self.calculate_time_distribution(result),
            #     "object_distribution": self.calculate_object_distribution(result)
            # }
        }
        
        # 子包统计
        for sub_packet_name, sub_packet_info in package_structure.sub_packets.items():
            detailed_stats["sub_packet_summary"][sub_packet_name] = {
                "sample_count": sub_packet_info["statistics"]["sample_count"],
                "time_range": sub_packet_info["statistics"]["time_range"],
                "camera_count": sub_packet_info["statistics"]["camera_count"]
            }
        
        with open(stats_file, 'w', encoding='utf-8') as f:
            json.dump(detailed_stats, f, indent=2, ensure_ascii=False, default=str)
        
        self.logger.info(f"📊 保存详细统计: {stats_file.name}")
        
        # 4. 保存无效样本报告
        if result.invalid_samples:
            invalid_file = self.analysis_dir / f"{result.package_name}_invalid_samples.json"
            
            # 按子包分组无效样本
            invalid_by_sub_packet = defaultdict(list)
            for sample in result.invalid_samples:
                invalid_by_sub_packet[sample.sub_packet_name].append(asdict(sample))
            
            invalid_data = {
                "package_name": result.package_name,
                "total_invalid": len(result.invalid_samples),
                "by_sub_packet": dict(invalid_by_sub_packet),
                "error_summary": self.summarize_invalid_errors(result.invalid_samples)
            }
            
            with open(invalid_file, 'w', encoding='utf-8') as f:
                json.dump(invalid_data, f, indent=2, ensure_ascii=False, default=str)
            
            self.logger.info(f"⚠️  保存无效样本报告: {invalid_file.name}")
        else:
            # 创建空报告
            invalid_file = self.analysis_dir / f"{result.package_name}_invalid_samples.json"
            with open(invalid_file, 'w', encoding='utf-8') as f:
                json.dump({"package_name": result.package_name, "total_invalid": 0, "by_sub_packet": {}}, f, indent=2)
            self.logger.info(f"✅ 无无效样本，创建空报告: {invalid_file.name}")
        
        # 5. 生成stage2的配置指南
        self.generate_stage2_guide(package_structure)


    def generate_stage2_guide(self, package_structure: PackageDataStructure):
        """生成stage2处理指南"""
        
        guide_file = self.analysis_dir / f"{package_structure.package_name}_stage2_guide.md"
        
        with open(guide_file, 'w', encoding='utf-8') as f:
            f.write(f"# {package_structure.package_name} - Stage2处理指南\n\n")
            f.write(f"**生成时间**: {package_structure.analysis_time}\n")
            f.write(f"**数据包类型**: {package_structure.package_type}\n")
            f.write(f"**总子包数**: {len(package_structure.sub_packets)}\n")
            
            # 统计信息
            total_samples = sum(len(sp["samples"]) for sp in package_structure.sub_packets.values())
            f.write(f"**总样本数**: {total_samples}\n\n")
            
            f.write("## 📁 数据结构\n\n")
            f.write("数据采用三层结构组织：\n")
            f.write("1. **Package Level**: 数据包级别信息\n")
            f.write("2. **Sub-packet Level**: 子包级别信息\n")  
            f.write("3. **Sample Level**: 样本详细信息\n\n")
            
            f.write("## 📊 子包详情\n\n")
            f.write("| 子包名称 | 样本数 | 时间范围 | 相机数 | 连续性 |\n")
            f.write("|----------|--------|----------|--------|--------|\n")
            ...
            for sub_packet_name, sub_packet_info in package_structure.sub_packets.items():
                ...
                stats = sub_packet_info["statistics"]
                time_range = stats["time_range"]
                
                # 时间范围格式化
                if time_range["min"] and time_range["max"]:
                    duration = (time_range["max"] - time_range["min"]) / 1e6
                    time_str = f"{duration:.1f}s"
                else:
                    time_str = "N/A"
                
                # 连续性判断
                continuity = stats.get("time_continuity", {})
                is_continuous = continuity.get("is_continuous", True)
                continuity_str = "✅" if is_continuous else "⚠️"
                
                f.write(f"| {sub_packet_name} | {len(samples)} | {time_str} | "
                    f"{stats['camera_count']} | {continuity_str} |\n")
            
            f.write("\n## 🎯 Stage2处理建议\n\n")
            f.write("### 1. 处理顺序\n")
            f.write("- 按子包顺序处理，每个子包独立处理\n")
            f.write("- 子包内样本按时间戳排序后处理\n\n")
            
            f.write("### 2. prev/next链构建\n")
            f.write("```python\n")
            f.write("# 子包内构建prev/next链\n")
            f.write("for sub_packet_name, sub_packet_info in data['sub_packets'].items():\n")
            f.write("    samples = sub_packet_info['samples']\n")
            f.write("    sorted_samples = sorted(samples, key=lambda x: x['metadata']['timestamp_int'])\n")
            f.write("    \n")
            f.write("    for i in range(len(sorted_samples)):\n")
            f.write("        if i > 0:\n")
            f.write("            sorted_samples[i]['prev_sample_in_sub_packet'] = sorted_samples[i-1]['unique_key']\n")
            f.write("        if i < len(sorted_samples) - 1:\n")
            f.write("            sorted_samples[i]['next_sample_in_sub_packet'] = sorted_samples[i+1]['unique_key']\n")
            f.write("```\n\n")
            
            f.write("### 3. 场景划分建议\n")
            f.write("根据时间连续性分析，建议的场景划分：\n")
            
            # 基于连续性建议场景划分
            for sub_packet_name, sub_packet_info in package_structure.sub_packets.items():
                continuity = sub_packet_info["statistics"].get("time_continuity", {})
                if not continuity.get("is_continuous", True):
                    f.write(f"- **{sub_packet_name}**: 存在时间断层，建议分割为多个场景\n")
            
            f.write("\n## 📋 关键配置\n\n")
            f.write("```json\n")
            f.write("{\n")
            f.write('  "camera_mapping": ' + json.dumps(package_structure.camera_mapping, indent=2) + ",\n")
            f.write('  "total_sub_packets": ' + str(len(package_structure.sub_packets)) + ",\n")
            f.write('  "total_samples": ' + str(total_samples) + "\n")
            f.write("}\n")
            f.write("```\n")
        
        guide_file.chmod(0o644)
        self.logger.info(f"📖 生成Stage2指南: {guide_file.name}")
