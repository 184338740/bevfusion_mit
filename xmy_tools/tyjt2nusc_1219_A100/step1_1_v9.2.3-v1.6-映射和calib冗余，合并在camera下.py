#!/usr/bin/env python3
"""
tyjt数据集转nuscenes格式转换工具 - v9.2.3-phase1-v1.6 (信息完整修复版)
版本: v9.2.3-phase1-v1.6-fullinfo-fixed
修改日期: 2024-12-25
核心改进:
...
class SubPacketInfo:
    ...
    name: str
    road_id: str
    calib_files: List[str] = field(default_factory=list)  # V1.6新增
...
class DataAnalyzerV1_6:
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger("analyzer_v1_6")
        ...
        # 文件处理器
        log_file = log_dir / "analysis_v1_6.log"
        ...
        formatter = logging.Formatter(
            '%(asctime)s - %(levelname)s - [v1.6] - %(message)s',
            datefmt='%Y-%m-%d %H:%M:%S'
        )
    ...
    def build_sample_info(self, package_name: str, package_type: str, road_id: str,
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"计算map2group矩阵失败: {e}")
                map2group_matrix = np.eye(4).tolist()
            
            # 激光雷达标定信息
            lidar_calib_key = f"{road_id}_Lidar"
            has_lidar_calib = False
            lidar_calibration = {
                "tyjt": {"sensor2map": {}, "sensor2map_matrix": []},
                "transform_matrices": {
                    "sensor2map": [], "map2group": [], "sensor2group": []
                },
                "nuscenes": {"translation": [], "rotation": []}
            }
            ...
            # 激光雷达标定处理
            if lidar_calib_key in calib_data:
                lidar_calib = calib_data[lidar_calib_key]
                has_lidar_calib = True
                
                # 解析激光雷达标定参数
                lidar_sensor2map_params = {}
                if isinstance(lidar_calib, dict):
                    if 'translation' in lidar_calib and 'rotation' in lidar_calib:
                        lidar_sensor2map_params = {
                            "translation": lidar_calib['translation'],
                            "rotation": lidar_calib['rotation']
                        }
                    elif all(k in lidar_calib for k in ['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'rw']):
                        lidar_sensor2map_params = {
                            "tx": lidar_calib['tx'],
                            "ty": lidar_calib['ty'],
                            "tz": lidar_calib['tz'],
                ...
                # 计算激光雷达变换矩阵
                try:
                    lidar_sensor2map_matrix = CalibrationProcessor.parse_transform(lidar_calib).tolist()
                    lidar_calibration["tyjt"]["sensor2map"] = lidar_sensor2map_params
                    lidar_calibration["tyjt"]["sensor2map_matrix"] = lidar_sensor2map_matrix
                    
                    if lidar_sensor2map_matrix and map2group_matrix:
                        lidar_sensor2map_np = np.array(lidar_sensor2map_matrix)
                        map2group_np = np.array(map2group_matrix)
                        lidar_sensor2group_np = map2group_np @ lidar_sensor2map_np
                        
                        lidar_calibration["transform_matrices"]["sensor2map"] = lidar_sensor2map_matrix
                        lidar_calibration["transform_matrices"]["map2group"] = map2group_matrix
                        lidar_calibration["transform_matrices"]["sensor2group"] = lidar_sensor2group_np.tolist()
                        
                        # 提取NuScenes格式的激光雷达标定
                        lidar_translation = lidar_sensor2group_np[:3, 3].tolist()
                        lidar_rotation = CalibrationProcessor.rotation_matrix_to_quaternion(lidar_sensor2group_np[:3, :3])
                        lidar_calibration["nuscenes"]["translation"] = lidar_translation
                        lidar_calibration["nuscenes"]["rotation"] = lidar_rotation
                except Exception as e:
                    self.logger.warning(f"处理激光雷达标定失败: {e}")
            ...
            for cam_config in road_config.get("cameras", []):
                if package_type == "hikvision":
                    if len(cam_config) >= 2:
                        ...
                        self.logger.warning(f"相机配置格式错误: {cam_config}")
                        continue
                else:
                    if len(cam_config) >= 3:
                ...
                if calibration_key in calib_data:
                    ...
                    if isinstance(tyjt_calib, dict):
                        ...
                        # 径向畸变参数
                        if 'k1' in tyjt_calib:
                            radial_distortion["k1"] = tyjt_calib.get('k1', 0.0)
                        if 'k2' in tyjt_calib:
                            radial_distortion["k2"] = tyjt_calib.get('k2', 0.0)
                        if 'k3' in tyjt_calib:
                            radial_distortion["k3"] = tyjt_calib.get('k3', 0.0)
                        if 'p1' in tyjt_calib:
                            radial_distortion["p1"] = tyjt_calib.get('p1', 0.0)
                        if 'p2' in tyjt_calib:
                            radial_distortion["p2"] = tyjt_calib.get('p2', 0.0)
                        ...
                        try:
                            ...
                            self.logger.warning(f"解析相机标定矩阵失败 {calibration_key}: {e}")
                            sensor2map_matrix = np.eye(4).tolist()
                    ...
                    if sensor2map_matrix is not None and map2group_matrix is not None:
                        try:
                            ...
                            self.logger.warning(f"计算相机变换矩阵失败 {calibration_key}: {e}")
    ...
    def save_package_result(self, result: PackageAnalysisResult):
        ...
        # 只保存一个三层结构数据文件
        main_file = self.analysis_dir / f"{result.package_name}_structured_v1_6.json"
    ...
    def generate_summary_report(self, all_results: List[PackageAnalysisResult]):
        ...
        # 保存汇总报告
        summary_file = self.analysis_dir / "summary_report_v1_6.json"
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)
        
        # 输出简要报告
        self.logger.info("\n" + "="*60)
        self.logger.info("📊 分析汇总报告 (v1.6)")
        self.logger.info("="*60)
...
def main():
    ...
    analyzer.logger.info("🔍 TYJT数据集分析工具 v9.2.3-phase1-v1.6 (信息完整修复版)")
    ...
    for result in all_results:
        if result:
            ...
            # 结构化数据文件
            structured_file = analyzer.analysis_dir / f"{result.package_name}_structured_v1_6.json"
            if structured_file.exists():
                analyzer.logger.info(f"    ✓ structured_v1_6.json: {result.overall_statistics['total_samples']} 个样本")
