...
if Mode == "Local":
    ...
    TYJT_ROOT = "/cephfsdata/users/lishan/00_Data/00_RawData"
    OUTPUT_ROOT = "./output-1226-v9.2.3-all/step1/nuscenes_tyjt"
    NUSC_VERSION = "v1.0-tyjt"
...
class FileCacheManager:
    ...
    def __init__(self):
        self._cache = {}  # package_name -> {sub_packet -> {timestamp -> file_info}}
        self._lock = threading.Lock()
    
    def scan_package(self, package_path: Path, package_config: Dict) -> Dict:
        """扫描整个数据包的文件结构（一次性）"""
        ...
        with self._lock:
            ...
            for sub_packet in datasets_dir.iterdir():
                ...
                file_structure[sub_packet_name] = self._scan_sub_packet(sub_packet, package_config)
            
            self._cache[cache_key] = file_structure
            return file_structure
    
    def _scan_sub_packet(self, sub_packet: Path, package_config: Dict) -> Dict:
        """扫描单个子包的所有文件"""
        timestamp_files = {}
        ...
        if lidar_dir.exists():
            for npy_file in lidar_dir.glob("*.npy"):
                ...
                # 设置lidar文件路径
                timestamp_files[timestamp]['lidar'] = str(npy_file.resolve())
        
        # 扫描label文件（基于已有的timestamp）
        label_dir = sub_packet / "lidar" / "label"
        if label_dir.exists():
            for json_file in label_dir.glob("*.json"):
                timestamp = json_file.stem
                if timestamp in timestamp_files:
                    timestamp_files[timestamp]['label'] = str(json_file.resolve())
        ...
        for road_id, road_config in package_config["intersections"].items():
            for cam_config in road_config.get("cameras", []):
                if package_config["type"] == "hikvision":
                    camera_folder = cam_config[0]
                    camera_order = cam_config[1]
                else:
                    camera_folder = cam_config[0]
                    camera_order = cam_config[2]
                
                camera_dir = sub_packet / camera_folder / "image_dc"
                if camera_dir.exists():
                    for jpg_file in camera_dir.glob("*.jpg"):
                        timestamp = jpg_file.stem
                        if timestamp in timestamp_files:
                            timestamp_files[timestamp]['cameras'][camera_order] = str(jpg_file.resolve())
        
        # 标记哪些timestamp有完整文件
        for timestamp, files in timestamp_files.items():
            # 检查必需文件：lidar + label + 至少2个相机
            has_lidar = files['lidar'] != ''
            has_label = files['label'] != ''
            has_enough_cameras = len(files['cameras']) >= 2
            
            files['has_all_files'] = has_lidar and has_label and has_enough_cameras
...
class DataAnalyzerV1_6:
    ...
    def __init__(self, output_root: str):
        ...
        self.calib_cache = {}
        self.file_cache = FileCacheManager()  # ✅ 新增文件缓存
        self.setup_logging()
    ...
    def analyze_package(self, package_dir: Path) -> PackageAnalysisResult:
        ...
        self.logger.info(f"开始扫描数据包: {package_name}")
        scan_start = time.time()
        
        # ✅ 先一次性扫描整个数据包的文件结构
        file_structure = self.file_cache.scan_package(package_dir, package_config)
        
        scan_time = time.time() - scan_start
        self.logger.info(f"文件扫描完成: {len(file_structure)} 个子包, 耗时 {scan_time:.2f}秒")
        ...
        for road_id, road_config in package_config["intersections"].items():
            ...
            # 处理每个子包
            for sub_packet_name, timestamp_files in file_structure.items():
                valid_samples = 0
                sub_packet_info = SubPacketInfo(
                    name=sub_packet_name,
                    road_id=road_id,
                    calib_files=[str(package_dir / package_config["calib_root"] / "*.json")]  # 示例
                )
                
                for timestamp, file_info in timestamp_files.items():
                    if not file_info.get('has_all_files', False):
                        continue
                    
                    # 从缓存获取文件路径
                    npy_path = Path(file_info['lidar'])
                    label_path = Path(file_info['label'])
                    sub_packet = npy_path.parent.parent.parent  # 获取子包路径
                    
                    # 使用现有的build_sample_info函数
                    sample = self.build_sample_info(
                        package_name, package_config["type"], road_id,
                        sub_packet_name, timestamp, npy_path,
                ...
                if sub_packet_info.samples:
                    ...
                    self.logger.info(f"子包 {sub_packet_name}: {valid_samples} 个有效样本")
    ...
    def build_sample_info(self, package_name: str, package_type: str, road_id: str,
        ...
        try:
            ...
            # 2. 优化：快速检查label文件（不读取完整内容）
            try:
                # 只需检查文件大小判断是否有数据
                file_size = label_file.stat().st_size
                if file_size > 100:  # 假设大于100字节的label文件可能有数据
                    label_objects = 1  # 设为默认值，分析阶段不需要精确计数
                    self.logger.debug(f"label文件大小: {file_size}字节")
                else:
                    self.logger.debug(f"label文件过小: {file_size}字节")
            except Exception as e:
                self.logger.debug(f"检查label文件失败 {label_file}: {e}")
                label_objects = 0
            ...
            # 7. 优化：激光雷达点云统计 - 不读取npy文件！
            try:
                if npy_file.exists():
                    # 使用文件大小估算点数（分析阶段不需要精确值）
                    file_size = npy_file.stat().st_size
                    # 假设每个点5个float32（x,y,z,intensity,ring = 20字节）
                    estimated_points = file_size // 20 if file_size > 0 else 0
                    lidar_point_count = estimated_points
                    self.logger.debug(f"估算激光雷达点数: {lidar_point_count} (文件大小: {file_size}字节)")
                    
                    # 分析阶段不需要实际的距离统计，保留默认值即可
            except Exception as e:
                self.logger.debug(f"获取npy文件信息失败 {npy_file}: {e}")
                lidar_point_count = 0
