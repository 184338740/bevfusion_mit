...
Mode = "A100"
if Mode == "Local":
    NUSC_ROOT = "./output-1226-v1.9/step1/nuscenes_tyjt"
    OUTPUT_DIR = "./output-1226-v1.9/step2_3"
    ...
    OUTPUT_DIR = "./output-1226-v9.2.3-all/step2_3_vis-v6"
...
MAX_SAMPLES_PER_SCENE = 10        # 每个场景最多处理的样本数量
...
def get_sampled_samples(nusc, sampling_strategy, max_samples_per_scene):
    ...
    sampled_samples = []
    problematic_scenes = 0  # 记录有问题的场景数量
    ...
    if sampling_strategy == "per_scene":
        ...
        for scene in nusc.scene:
            ...
            # ========== 关键修复：检查first_sample_token是否有效 ==========
            if not first_sample_token or first_sample_token.strip() == "":
                print(f"   ⚠️  跳过场景 {scene_name}: 第一个样本token为空")
                problematic_scenes += 1
                continue
            # ========== 修复结束 ==========
            ...
            try:
                ...
                while current_sample is not None:
                    ...
                    # ========== 修复：安全获取下一个token ==========
                    next_token = current_sample.get('next', '')
                    if not next_token or next_token.strip() == "":
                        break  # 没有下一个样本了
                        
                    try:
                        current_sample = nusc.get('sample', next_token)
                    except KeyError:
                        break  # 如果找不到下一个样本，结束循环
                    # ========== 修复结束 ==========
                ...
                print(f"   ❌ 场景 {scene_name}: 处理失败 - {e}")
                problematic_scenes += 1
                continue
        
        if problematic_scenes > 0:
            print(f"   ⚠️  跳过 {problematic_scenes} 个有问题的场景")
...
def main():
    ...
    # ========== 新增：数据路径验证 ==========
    print("\n🔍 验证数据路径...")
    nusc_root_path = Path(NUSC_ROOT)
    
    if not nusc_root_path.exists():
        print(f"❌ 数据根目录不存在: {nusc_root_path}")
        print(f"   请检查路径是否正确")
        return
    
    print(f"✅ 数据根目录存在: {nusc_root_path}")
    
    # 检查关键目录
    required_dirs = ["samples", "sweeps", "maps", f"v1.0-tyjt"]
    for dir_name in required_dirs:
        dir_path = nusc_root_path / dir_name
        if dir_path.exists():
            print(f"✅ {dir_name}: 存在")
        else:
            print(f"⚠️  {dir_name}: 不存在 - 这可能导致数据加载失败")
    
    # 检查标注文件
    annotation_path = nusc_root_path / "v1.0-tyjt" / "sample_annotation.json"
    ...
    try:
        ...
        nusc = NuScenes(
            ...
            dataroot=NUSC_ROOT, 
            verbose=True,
            map_resolution=0.1  # 添加默认参数
        )
        
        print(f"✅ NuScenes加载成功！")
        print(f"   样本数量: {len(nusc.sample)}")
        print(f"   场景数量: {len(nusc.scene)}")
        print(f"   类别数量: {len(nusc.category)}")
        
        if len(nusc.sample) == 0:
            print(f"⚠️  警告：样本数为0，可能是数据格式问题")
            return
        
        # 显示前几个场景信息（避免输出太长）
        print(f"\n📋 场景列表（显示前10个）：")
        for i, scene in enumerate(nusc.scene[:10]):
            sample_count = 0
            try:
                ...
                while current_sample is not None:
                    ...
                    if current_sample['next']:
                        ...
                        break
                print(f"   {i+1:2d}. {scene['name']}: {sample_count} 个样本")
            except:
                print(f"   {i+1:2d}. {scene['name']}: 无法统计样本数")
        
        if len(nusc.scene) > 10:
            print(f"   ... 还有 {len(nusc.scene) - 10} 个场景未显示")
        ...
        # 检查可能的具体问题
        print(f"\n🔍 诊断信息:")
        metadata_path = nusc_root_path / NUSC_VERSION / "sample.json"
        if metadata_path.exists():
            print(f"   元数据文件大小: {metadata_path.stat().st_size} 字节")
        else:
    ...
    try:
        ...
        for folder in output_path.iterdir():
            if folder.is_dir():
                png_files = len(list(folder.rglob('*.png')))
                json_files = len(list(folder.rglob('*.json')))
                if png_files + json_files > 0:
                    print(f"   📁 {folder.name}/ - {png_files} PNG + {json_files} JSON")
