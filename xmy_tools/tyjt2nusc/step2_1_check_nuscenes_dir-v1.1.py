...
def check_json_file(json_file: str) -> bool:
    ...
    # 7. 验证字段值类型（核心修改：新增calibrated_sensor分类型校验）
    value_types = schema["value_types"]
    # 加载sensor.json获取传感器类型映射（仅用于calibrated_sensor校验）
    sensor_modality_map = {}
    if json_file == "calibrated_sensor.json":
        sensor_path = os.path.join(JSON_DIR, "sensor.json")
        try:
            with open(sensor_path, 'r', encoding='utf-8') as f:
                sensor_data = json.load(f)
            sensor_modality_map = {s["token"]: s["modality"] for s in sensor_data}
        except Exception as e:
            print(f"❌ 加载sensor.json失败: {str(e)}")
            return False
    
    for i, item in enumerate(data[:5]):
        for key, vtype in value_types.items():
            val = item[key]
            # 针对calibrated_sensor.json的camera_intrinsic特殊处理
            if json_file == "calibrated_sensor.json" and key == "camera_intrinsic":
                # 获取传感器类型（lidar/camera）
                sensor_token = item["sensor_token"]
                modality = sensor_modality_map.get(sensor_token, "")
                # 相机：必须是3x3矩阵
                if modality == "camera":
                    if not isinstance(val, list) or len(val) != 3:
                        print(f"❌ 第{i}项（相机）'camera_intrinsic'长度错误: 预期3，实际{len(val) if isinstance(val, list) else '非列表'}")
                        return False
                    for row in val:
                        if not isinstance(row, list) or len(row) != 3 or not all(isinstance(elem, float) for elem in row):
                            print(f"❌ 第{i}项（相机）'camera_intrinsic'元素错误: 预期3个float元素的列表")
                            return False
                # 激光雷达：必须是空列表
                elif modality == "lidar":
                    if not isinstance(val, list) or len(val) != 0:
                        print(f"❌ 第{i}项（激光雷达）'camera_intrinsic'错误: 预期空列表，实际{len(val) if isinstance(val, list) else '非列表'}")
                        return False
                # 其他传感器：默认空列表
                else:
                    if not isinstance(val, list) or len(val) != 0:
                        print(f"❌ 第{i}项（{modality}）'camera_intrinsic'错误: 预期空列表")
                        return False
                continue  # 跳过后续默认校验
            
            # 处理普通复杂类型（如：3个float元素的列表）
