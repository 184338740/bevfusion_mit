    ...
    def __init__(self,
        ...
        if pred_tokens != gt_tokens:
            ...
            for token in common_tokens:
                # 保留预测框中这个token的数据
                if token in self.pred_boxes:
                    filtered_pred_dict[token] = self.pred_boxes[token]
                
                # 保留GT框中这个token的数据
                if token in self.gt_boxes:
                    filtered_gt_dict[token] = self.gt_boxes[token]
            
            # 用过滤后的数据创建新的EvalBoxes对象
            self.pred_boxes = EvalBoxes(filtered_pred_dict)
            self.gt_boxes = EvalBoxes(filtered_gt_dict)
            # ============ 关键修改结束 ============
            """
