# /data2/xmy/01_project/bevfusion_mit_xmy_1205/mmdet3d/datasets/nuscenes_dataset.py
import tempfile
...
class NuScenesDataset(Custom3DDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        print(f">>>[xmy]🔵[0] bevfusion_mit_xmy/mmdet3d/datasets/nuscenes_dataset.py:206 load_annotations(); self.version  = {self.version}")
        return data_infos
    ...
    def _evaluate_single(
        ...
        print(f">>>[xmy]🔵[0] bevfusion_mit_xmy/mmdet3d/datasets/nuscenes_dataset.py:435 _evaluate_single(); self.version= {self.version}")
        ...
        if self.version in eval_set_map:
            ...
            # 默认使用val
            eval_set = 'val'
            print(f'🔵[TYJT警告]>>> 未知版本 {self.version}, 使用默认eval_set: {eval_set}')
        
        print(f'🔵[TYJT调试]>>> 使用eval_set: {eval_set}, 数据集版本: {self.version}')
        ...
        nusc_eval = DetectionEval(
            ...
            verbose=False,  # 改为True以显示详细调试信息
        )
        nusc_eval.main(render_curves=False)

        # 记录指标
        print(f'🔵[TYJT调试]>>> 保存: output_dir = {output_dir}')
