...
def main():
    ...
    parser.add_argument('--max-sweeps', type=int, default=10,
                        help='Number of sweeps (previous frames) to include for each sample')
    # 新增参数
    parser.add_argument('--generate-info', default='true',
                        help='Generate tyjt_infos_train.pkl and tyjt_infos_val.pkl')
    parser.add_argument('--generate-database', default='true',
                        help='Generate GT database (tyjt_dbinfos_train.pkl and tyjt_gt_database/)')
    ...
    if args.generate_info:
        ...
        print("跳过生成 info 文件 (需要 --generate-info 才会生成)")
    ...
    if args.generate_database:
        if not train_pkl.exists():
            print(f"错误：info 文件 {train_pkl} 不存在，无法生成数据库。请先使用 --generate-info 生成 info 文件。")
        else:
            print(" ========== 开始生成 GT 数据库... ========== ")
            create_tyjt_gt_database(str(train_pkl), out_dir, str(data_root))
            print(" ========== GT 数据库生成完成。========== ")
    else:
        print("跳过生成 GT 数据库 (需要 --generate-database 才会生成)")
