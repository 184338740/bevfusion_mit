...
if Mode == "Local":
    ...
    OUTPUT_ROOT = "./output-1204-sub/step1/nuscenes_tyjt"
    NUSC_VERSION = "v1.0-tyjt"
...
def create_directory_structure(output_root: str, version: str):
    ...
    try:
        ...
        # 如果没有PIL，创建空文件
        with open(maps_dir / "empty_map.png", 'wb') as f:
            f.write(b'')
...
def process_single_sample(
    ...
    """处理单个样本：图像软链接，LIDAR bin生成（固定4相机）- 优化版本"""
...
def process_sensor_data(
    ...
    """处理传感器数据：固定4相机软链接，LIDAR bin生成 - 优化版本"""
    ...
    # 如果bin文件已经存在，检查是否所有输出文件都已存在
    output_files_exist = False
    if fast_checker.exists(lidar_output_path):
        # 检查所有相机图像软链接是否都已存在
        all_images_exist = True
        for cam_info in intersection_config["cameras"]:
            cam_order = cam_info[-1]  # 最后一个元素是相机顺序标识
            nusc_cam_name = CAM_ORDER_TO_NUSC.get(cam_order)
            if nusc_cam_name:
                img_output_path = output_root / "samples" / nusc_cam_name / f"{unique_key}.jpg"
                if not fast_checker.exists(img_output_path):
                    all_images_exist = False
                    break
        
        if all_images_exist:
            package_logger.debug(f"跳过已处理的样本: {timestamp}")
            # 但仍需要创建sample_data记录
    ...
    for cam_info in intersection_config["cameras"]:
        ...
        if not output_files_exist:
            # 检查源文件是否存在（使用缓存）
            if not fast_checker.exists(img_original_path):
            ...
            package_logger.debug(f"使用已存在的软链接: {img_output_path}")
    ...
    if not output_files_exist:
        # 检查源文件是否存在（使用缓存）
        if not fast_checker.exists(lidar_original_npy):
        ...
        if not fast_checker.exists(lidar_output_path):
        ...
        package_logger.debug(f"使用已存在的bin文件: {lidar_output_path.name}")
...
def process_annotation_data(
    ...
    # 使用缓存检查器
    if not fast_checker.exists(label_path):
...
def process_intersection_samples(
    ...
    """处理单个路口的所有样本（固定4相机）- 优化版本：批量扫描"""
    datasets_dir = package_dir / "datasets"
    # 使用缓存检查器
    if not fast_checker.exists(datasets_dir):
    ...
    try:
        ...
        package_logger.error(f"扫描子包失败: {e}")
        return []
    
    package_logger.info(f"扫描 {len(sub_packets)} 个子包...")
    
    # 批量预扫描：一次性收集所有有效样本
    valid_samples_by_subpacket = defaultdict(list)
    
    for sub_packet_idx, sub_packet in enumerate(sub_packets):
        package_logger.debug(f"预扫描子包 [{sub_packet_idx+1}/{len(sub_packets)}]: {sub_packet.name}")
        
        lidar_dir = sub_packet / "lidar" / "pcd"
        # 使用缓存检查器
        if not fast_checker.exists(lidar_dir):
        ...
        try:
            ...
            for lidar_npy in lidar_npy_files:
                ...
                # 检查相机文件（可选，可以先不检查，处理时再检查）
                valid_samples_by_subpacket[sub_packet].append((lidar_npy, timestamp))
            ...
            package_logger.error(f"扫描子包 {sub_packet.name} 失败: {e}")
            continue
    
    # 将所有有效样本收集到同一个列表并排序
    for sub_packet, samples in valid_samples_by_subpacket.items():
        for lidar_npy, timestamp in samples:
    ...
    package_logger.info(f"路口 {intersection_key} 处理完成: {processed_count} 个样本, "
                       f"耗时: {total_time:.2f}秒, 平均速度: {processed_count/total_time:.1f} 样本/秒")
...
def validate_data_consistency(nusc_data: Dict, output_root: Path) -> Tuple[bool, List[str]]:
    ...
    # 批量检查（减少系统调用）
    exist_results = fast_checker.batch_exists(check_paths)
    
    for i, (sample_data, file_path) in enumerate(zip(sample_data_records, check_paths)):
        file_key = sample_data['filename']
        
        if not exist_results.get(file_path, False):
...
def main():
    ...
    main_logger.info("=" * 60)
    main_logger.info("🚀 Step1: TYJT→NuScenes转换工具 v9.1（分布式存储优化版）")
    ...
    main_logger.info(f"⚡ 分布式存储优化: 启用文件存在性检查缓存 (TTL=300s)")
    main_logger.info("=" * 60)
    ...
    # 4. 多线程处理数据包 - 减少线程数以避免分布式存储拥塞
    main_logger.info("开始多线程处理数据包...")
    package_start_time = time.time()
    
    # 对于分布式存储，减少线程数以避免I/O拥塞
    max_workers = max(2, min(len(packages), 4))  # 2-4个线程
    main_logger.info(f"使用 {max_workers} 个线程处理 {len(packages)} 个数据包")
    ...
    main_logger.info(f"总耗时: {total_elapsed_time:.2f}秒")
    ...
    if not data_valid:
        main_logger.warning(f"  注意: 数据存在 {len(errors)} 个一致性问题")
