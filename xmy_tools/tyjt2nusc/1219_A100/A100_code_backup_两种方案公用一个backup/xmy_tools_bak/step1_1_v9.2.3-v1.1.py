...
class EnhancedDataAnalyzer:
    ...
    def create_validated_sample(self, package_name: str, package_type: str, road_id: str,
        ...
        for camera_calib in calibration_set.cameras:
            nuscenes_calibration["sensors"][camera_calib.nusc_camera_name] = {
    ...
    def save_enhanced_analysis_result(self, result: AnalysisResult):
        ...
        if result.time_analysis:
            ...
            for scene in result.time_analysis.suggested_scenes:
                scene_dicts.append(asdict(scene))
            ...
            for sample in result.valid_samples:
                ...
                sample_dict["processing_info"] = {
                    ...
                    "nuscenes_camera_names": [
                        cam.get("nusc_camera_name", "")
                        for cam in sample.files.get("cameras", {}).values()
...
def generate_phase2_script(analysis_dir: Path, summary: Dict):
    ...
    package_names = []
    for pkg_detail in summary['package_details']:
        package_names.append(pkg_detail['package_name'])
...
分析统计:
- 总数据包数: {summary['analysis_summary']['total_packages']}
- 总有效样本数: {summary['analysis_summary']['total_valid_samples']}
- 总无效样本数: {summary['analysis_summary']['total_invalid_samples']}
- 建议场景数: {summary['analysis_summary']['total_suggested_scenes']}
- 总体质量: {summary['quality_assessment']['overall_quality']}
- 生成时间: {summary['analysis_summary']['analysis_start_time']}
"""
...
print("=" * 60)
print("🚀 TYJT数据集转换 - 阶段2启动")
print("=" * 60)

# 包名列表
PACKAGE_NAMES = {json.dumps(package_names)}

print(f"📊 总有效样本数: {summary['analysis_summary']['total_valid_samples']}")
print(f"📊 建议场景数: {summary['analysis_summary']['total_suggested_scenes']}")

# 建议运行命令
print("\\n💡 建议运行命令:")
print(f"python step1_2_v9.2.3-phase2.py --analysis-dir {{analysis_dir}} --output {{OUTPUT_ROOT}}")
print("\\n📖 详细文档请参考: analysis_report.md")
print("=" * 60)
...
'''
    ...
    logger.info(f"📜 生成阶段2启动脚本: {script_file}")
