...
class CameraInfo:
    ...
    calibration: Dict[str, Any] = field(default_factory=lambda: {
        "tyjt": {},  # 原始TYJT标定
        "nuscenes": {}  # 转换后的NuScenes标定
    })
    
    def __post_init__(self):
        if not self.nuscenes_name:
            self.nuscenes_name = CAM_ORDER_TO_NUSC.get(self.order, "")
...
class PackageAnalysisResult:
    ...
    package_type: str
    analysis_time: str
    sub_packets: Dict[str, SubPacketInfo] = field(default_factory=dict)  # name -> SubPacketInfo
        ...
        try:
            ...
            for cam_config in road_config["cameras"]:
                ...
                if calibration_key in calib_data and group_key in calib_data:
                    translation, rotation, calib_detail = CalibrationProcessor.compute_sensor2ego(
                        calib_data, calibration_key, group_key
                    )
                    
                    # 存储标定信息 (统一格式)
                    camera_info.calibration = {
                        "tyjt": calib_data[calibration_key] if calibration_key in calib_data else {},
                        "nuscenes": {
                            "translation": translation,
                            "rotation": rotation,
                            "detail": calib_detail
            ...
            sample = SampleInfo(
                ...
                calibration_summary={
                    "group_key": group_key,
                    "has_all_calibrations": all(cam.calibration.get("nuscenes", {}).get("is_valid", False) 
