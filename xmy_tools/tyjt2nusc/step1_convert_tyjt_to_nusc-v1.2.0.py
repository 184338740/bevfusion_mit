...
def transform_sensor_to_ego(sensor_calib, group2map_calib):
    ...
    try:
        ...
        # 2. 创建四元数对象
        sensor_q = Quaternion(sensor_quat)
        group_q = Quaternion(group_quat)
        
        # 3. 计算 map2group 变换（group2map的逆）
        map2group_q = group_q.inverse
        
        # 4. 计算 sensor2group 旋转
        # sensor2group = map2group × sensor2map
        sensor2group_q = map2group_q * sensor_q
        
        # 5. 计算 sensor2group 平移
        # 相对平移 = sensor_trans - group_trans
        relative_trans = sensor_trans - group_trans
        # sensor2group_trans = map2group_q.rotate(relative_trans)
        sensor2group_trans = map2group_q.rotation_matrix @ relative_trans
        
        print(f"🔄 传感器标定转换:")
        print(f"   sensor2map: 位置{sensor_trans}, 旋转{sensor_quat}")
        print(f"   group2map:  位置{group_trans}, 旋转{group_quat}")
        print(f"   sensor2group: 位置{sensor2group_trans}, 旋转{sensor2group_q.elements}")
        
        return sensor2group_trans.tolist(), sensor2group_q.elements.tolist()
