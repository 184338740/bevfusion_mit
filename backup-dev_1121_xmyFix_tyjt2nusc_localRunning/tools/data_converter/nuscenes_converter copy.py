...
def create_nuscenes_infos(root_path,
    ...
    # 1. 初始化NuScenes对象和场景（train/val/test）划分
    print(f"🔵[xmy]>>> nuscenes_converter.create_nuscenes_infos() 开始")
    print(f"  root_path: {root_path}")
    print(f"  version: {version}")
    nusc = NuScenes(version=version, dataroot=root_path, verbose=True)
    print(f"🔵[xmy]>>> nusc={nusc}")
    print(f"🔵[xmy]>>> NuScenes初始化成功，继续执行...")
    ...
    # available_vers = ['v1.0-trainval', 'v1.0-test', 'v1.0-mini', 'v1.0-tyjt-trainval', 'v1.0-tyjt-test', 'v1.0-tyjt']
    ...
    if version in available_vers:
        ...
        # 🔵[xmy]>>> 自定义版本（如tyjt）使用自动划分
        print(f"🔵[xmy]>>> 自定义版本: {version}，使用自动场景划分")
        
        # 正确的方法：使用nusc对象的get_scenes()方法
        scenes = nusc.scene
        scene_tokens = [s['token'] for s in scenes]
        
        # 🔵[xmy]>>> 冗余处理：处理单场景情况
        if len(scene_tokens) == 1:
            print(f"🔵[xmy]>>> 单场景数据集，训练集和验证集使用同一场景")
            train_scenes = set(scene_tokens)  # 训练集包含该场景
            val_scenes = set(scene_tokens)    # 验证集也包含该场景
        elif len(scene_tokens) == 2:
            print(f"🔵[xmy]>>> 双场景数据集，各用一个场景")
            train_scenes = set([scene_tokens[0]])
            val_scenes = set([scene_tokens[1]])
        else:
            # 多场景数据集：70%训练，30%验证
        ...
        print(f"🔵[xmy]>>> 自动划分: {len(train_scenes)}训练场景, {len(val_scenes)}验证场景")

        
    # 2. 场景过滤和处理
    # filter existing scenes.
    available_scenes = get_available_scenes(nusc)  # 获取可用的场景（train/val/test）
    ...
    # 3. 核心信息填充（这是核心函数，会为每个样本生成详细的信息字典。）【主函数】
...
def _fill_trainval_infos(nusc,
    ...
    i_ = 0
    print(f"🔵[xmy]>>> bevfusion_mit_xmy/tools/data_converter/nuscenes_converter.py::278::_fill_trainval_infos() 遍历样本 ")
    print(f"""🔵[xmy]>>> 
            (0~1) 初始化&填充, 基础信息
            (2) 填充, 标定信息
            (3) 填充, cam信息     
            (4) 填充, radar信息
            (5) 填充, Gt信息标签 gt_boxes, gt_names, gt_velocity""")

    import pdb; pdb.set_trace()
    # for sample in mmcv.track_iter_progress(nusc.sample):
    for ii, sample in enumerate(mmcv.track_iter_progress(nusc.sample)):
        # i_ += 1 
        # if i_ > 6: 
        #     break 
        ...
        ii == 0 and print(f"🔵[xmy]>>> (0~1) 初始化&填充, 基础信息 ")
        info = {
        ...
        ii == 0 and print(f"🔵[xmy]>>> (2) 填充, 标定信息")
        ...
        ii == 0 and print(f"🔵[xmy]>>> (3) 填充, 相机信息")
        ...
        ii == 0 and print(f"🔵[xmy]>>> (4) 填充, radar信息")
        ...
        # obtain annotation
        if not test:
            ii == 0 and print(f"🔵[xmy]>>> (5) 填充, Gt信息标签 gt_boxes, gt_names, gt_velocity")
            annotations = [
            ...
            # 获取3D gt_box:  list[(x, y, z, length, width, height, -yaw - π/2)],
