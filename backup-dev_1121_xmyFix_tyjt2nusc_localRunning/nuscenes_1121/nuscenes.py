...
class NuScenes:
    ...
    def box_velocity(self, sample_annotation_token: str, max_time_diff: float = 1.5) -> np.ndarray:
        ...
        # 🔵[xmy临时修改]>>> [tyjt数据适配][相机缺失问题] 容错处理： tyjt 4个相机的适配，缺少 CAM_BACK_LEFT、CAM_BACK_RIGHT
        # has_prev = current['prev'] != ''
        # has_next = current['next'] != ''
        # print(f" 🔵[xmy临时修改]>>> /opt/venv/lib/python3.8/site-packages/nuscenes/nuscenes.py >>> 临时修改，适配tyjt数据的速度问题")
        has_prev = 'prev' in current and current['prev'] != ''
        has_next = 'next' in current and current['next'] != ''
