#!/usr/bin/env python3
"""
TYJT 数据集异常帧对检测工具（v06）
基于 info pkl 文件，检测相邻帧目标数量突变（增加或减少）超过阈值的帧对，
并为每个异常帧对生成可视化图像（相机带框图 + BEV 图）和 info.txt 记录。

改进点（v06）：
- 修复 v04/v05 中变量 curr_info 未定义的错误。
- 汇总报告增加每个异常帧对的 base_name 信息，优化结构，提高可读性。

使用说明：
python tyjt_tools_valid_detector_v043_0325.py --info-pkl <pkl文件> --detect-out <输出目录> [可选参数]

用例：
# tag-val
python tyjt_tools_valid_detector_v043_0325.py \
    --info-pkl ./tyjt_data_infos_v0942/tyjt_infos_val.pkl \
    --detect-out ./output/my_detectResults_Error_val_v043 \
    --thresh-ratio 0.2 \
    --thresh-abs 10 \
    --x-range -60 60 \
    --y-range -60 60
...
# tag-train
python tyjt_tools_valid_detector_v043_0325.py \
    --info-pkl ./output/tyjt_data_infos_v0942/tyjt_infos_train.pkl \
    --detect-out ./output/my_detectResults_Error_train_v043 \
    --thresh-ratio 0.2 \
    --thresh-abs 10 \
    --x-range -60 60 \
    --y-range -60 60
...
def get_primary_change_category(prev_stats, curr_stats):
    ...
    找出变化绝对值最大的类别（主要突变类别）。
    参数：
        prev_stats, curr_stats: dict, 类别->数量
    返回：
        (category, change, change_type)  change_type 为 'increase' 或 'decrease'
        若无变化或所有类别变化均为0，则返回 (None, 0, None)
    """
...
def detect_missing_labels(info_pkl, out_dir, thresh_ratio, thresh_abs,
    ...
    """
    检测标注缺失的帧对（objects 数量突变），并生成可视化文件及汇总报告。
    参数：
    ...
    for idx, info in enumerate(infos):
        ...
        if prev_num > 0 or curr_num > 0:
            ...
            if ratio >= thresh_ratio and diff >= thresh_abs:
                ...
                # 注意：这里使用 info 作为当前帧信息，而不是未定义的 curr_info
    ...
    # 生成整体汇总报告
    generate_summary(out_root, problem_pairs)
...
def generate_summary(out_root, problem_pairs):
    """
    生成整体汇总报告 summary.txt，包含所有异常帧对的摘要信息，每个异常帧对为一个区块，
    包含前后帧索引、base_name、数量变化、主要突变类别等，提高可读性。
    """
    summary_path = out_root / "summary.txt"
    with open(summary_path, 'w') as f:
        f.write("TYJT 数据集异常帧对检测汇总报告\n")
        f.write("=" * 80 + "\n\n")
    ...
    print(f"整体汇总报告已保存至: {summary_path}")


def main():
    parser = argparse.ArgumentParser(description="TYJT 数据集异常帧对检测工具（v06）")
