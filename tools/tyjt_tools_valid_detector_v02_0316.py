...
使用说明：
python tyjt_tools_valid_detector_v02_0316.py --info-pkl <pkl文件> --detect-out <输出目录> [可选参数]

用例:
# tag-val
python tyjt_tools_valid_detector_v02_0316.py \
    ...
    for i, (prev_idx, curr_idx, prev_num, curr_num, direction,
        ...
        for role, info, base in [('prev', prev_info, prev_base), ('curr', curr_info, curr_base)]:
            ...
            for cam in ['CAM_A', 'CAM_B', 'CAM_C', 'CAM_D']:
                if cam in info['cams']:
                    # 新格式使用 data_path
                    src_img = info['cams'][cam]['data_path']
                    if Path(src_img).exists():
                        img = cv2.imread(src_img)
                        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

                        # 构造临时相机信息，将新字段映射为旧版投影函数需要的字段
        ...
        with open(pair_dir / "info.txt", 'w') as f:
            ...
            for cam in ['CAM_A', 'CAM_B', 'CAM_C', 'CAM_D']:
                # 注意：info.txt 中记录的是原始路径，新格式下仍可以使用 data_path 或其他字段
                # 为保持一致性，这里仍然使用 data_path，但字段名可以自定义
