...
def visualize_sample(sample_idx: int, info: Dict, out_dir: Path):
    ...
    for cam in CAM_NAMES:
        ...
        for i, box in enumerate(boxes_3d):
            ...
            # 获取相机变换（与 project_box_to_image 中一致）
            t_cam2ego = np.array(info['cams'][cam]['cam2ego_translation'])
            q = np.array(info['cams'][cam]['cam2ego_rotation'])
            R_cam2ego = quaternion_to_rotation_matrix(q)
            R_ego2cam = R_cam2ego.T
            t_ego2cam = -R_ego2cam @ t_cam2ego
            ...
            z = center_cam[0, 2]
            if z > 0:
                intrinsic = np.array(info['cams'][cam]['cam_intrinsic'])
