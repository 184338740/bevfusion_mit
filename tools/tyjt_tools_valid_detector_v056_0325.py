...
- 基于突变点分段的低标注段检测：段内平均目标数 < low_avg_thresh 即视为低标注段
- 目标数量曲线图生成
- 详细日志输出
"""
...
def visualize_boundary_frames(out_root, segment_idx, start_idx, end_idx,
    ...
    try:
        with open(info_txt, 'w') as f:
            ...
            f.write(f"段内每帧目标数: {segment_stats['counts']}\n")
            f.write(f"低标注段判定阈值: 平均目标数 < {segment_stats['low_avg_thresh']}\n")
            f.write("\n边界帧说明:\n")
...
def generate_summary(out_root, problem_pairs, low_segments):
    ...
    try:
        with open(summary_path, 'w') as f:
            ...
            # 低标注段统计
            f.write("【基于突变点分段的低标注段检测结果】\n")
            f.write(f"总计低标注段数量: {len(low_segments)}\n")
            if low_segments:
                f.write("详细低标注段列表:\n")
                f.write("-" * 80 + "\n")
                for i, (start, end, avg, counts) in enumerate(low_segments):
                    f.write(f"\n【低标注段 #{i:04d}】\n")
                    f.write(f"  起始帧索引: {start}, 结束帧索引: {end}\n")
                    f.write(f"  段长度: {end - start + 1} 帧\n")
                    f.write(f"  段内平均目标数: {avg:.2f}\n")
                    f.write(f"  段内每帧目标数: {counts}\n")
                    f.write("  " + "-" * 76 + "\n")
            else:
...
def detect_missing_labels(info_pkl, out_dir, thresh_ratio, thresh_abs,
    ...
    if not split_points:
        ...
        # 判断每个区间是否为低标注段（段内平均目标数 < low_avg_thresh）
        low_segments = []
        for start, end in intervals:
            seg_counts = counts[start:end+1]
            avg_count = np.mean(seg_counts)
            if avg_count < low_avg_thresh:
                low_segments.append((start, end, avg_count, seg_counts))
                log_info(f"低标注段: [{start},{end}] 平均={avg_count:.2f}, 每帧={seg_counts}")

        log_info(f"发现 {len(low_segments)} 个低标注段")
        # 生成可视化
        for seg_idx, (start, end, avg_count, seg_counts) in enumerate(low_segments):
    ...
    # 汇总报告
    generate_summary(out_root, problem_pairs, low_segments)
    log_info("检测完成")

def main():
    parser = argparse.ArgumentParser(description="TYJT 数据集异常检测工具（v057）")
    ...
    parser.add_argument('--low-avg-thresh', type=float, default=5.0,
                        help='低标注段判定阈值：段内平均目标数低于此值视为低标注段')
    ...
    detect_missing_labels(
        ...
        low_avg_thresh=args.low_avg_thresh,
