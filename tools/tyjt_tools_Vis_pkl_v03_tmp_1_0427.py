...
def project_points_to_image(points: np.ndarray, cam_info: Dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if np.any(valid):
        ...
        uv[valid, 0] = u
        uv[valid, 1] = v
    return uv, valid, points_cam[:, 2]  # 返回深度
...
def draw_projected_points(image, uv, valid, depths=None, point_size_base=2):
    """
    uv: (N,2) 投影坐标
    valid: (N,) 有效标志（z>0）
    depths: (N,) 每个点的深度（z 值），用于颜色映射
    """
    img_copy = image.copy()
    ...
    if depths is not None:
        ...
        # 深度归一化（例如 0~80 米），更远的点更暗/更蓝
        d_norm = np.clip(d / 80.0, 0, 1)
        # 颜色映射：近处红色(255,0,0) -> 远处蓝色(0,0,255)
        colors = (np.stack([(1 - d_norm) * 255, np.zeros_like(d_norm), d_norm * 255], axis=1)).astype(np.uint8)
        # 点大小随距离减小（近大远小）
        point_sizes = np.clip(point_size_base * (1.5 - d_norm), 1, point_size_base * 2).astype(int)
    else:
        colors = [(0, 255, 0)] * len(u)
        point_sizes = [point_size_base] * len(u)

    for i, (x, y) in enumerate(zip(u, v)):
        cv2.circle(img_copy, (x, y), point_sizes[i], colors[i].tolist(), -1)
    return img_copy

def draw_projected_box(image: np.ndarray, uv: np.ndarray, valid: np.ndarray,
                       box_type: str = None, thickness: int = 2, line_type: int = cv2.LINE_AA) -> np.ndarray:
    """
    绘制投影的3D框。
    Args:
        color: BGR 颜色元组，如果 box_type 提供且颜色映射存在，则覆盖。
        box_type: 类别名称，用于从 OBJECT_PALETTE 获取颜色。
    """    """在图像上绘制投影的 3D 框（线框）"""
    if box_type and box_type in OBJECT_PALETTE:
        color_rgb = OBJECT_PALETTE[box_type]
...
def visualize_bev_nuscenes_style(out_path: Path, points: np.ndarray,
                                 boxes_3d: List[Dict], xlim=(-100,100), ylim=(-100,100)):
    """
    绘制类似 NuScenes 风格的 BEV 图：
    - 黑色背景
    - 点云按高度着色（z值）
    - 3D 框按类别颜色绘制（带前向线）
    """
    ...
    # 按高度着色：假设高度范围 -2 ~ 4 米，可以根据实际点云范围调整
    z = points[:, 2]
    z_min, z_max = -2, 4
    z_norm = (z - z_min) / (z_max - z_min)
    z_norm = np.clip(z_norm, 0, 1)
    colors = plt.cm.jet(z_norm)  # 使用 jet 色图，近红远蓝
    ax.scatter(points[:, 0], points[:, 1], s=1.5, c=colors, alpha=0.8, edgecolors='none')
    ...
    for box in boxes_3d:
        ...
        ax.plot(corners[:,0], corners[:,1], linewidth=8, color=color_norm, solid_capstyle='round')
        # 方向线（车头指向）
        front = np.array([l/2, 0]) @ R.T + np.array([cx, cy])
        ax.plot([cx, front[0]], [cy, front[1]], linewidth=8, color=color_norm)

        # 可选：在中心显示类别和ID
        # ax.text(cx, cy, f"{obj_type}_{box['id']}", fontsize=6, color='white')
...
def visualize_sample(sample_idx: int, info: Dict, out_dir: Path):
    ...
    for cam in CAM_NAMES:
        ...
        # 投影点云
        uv_pts, valid_pts, depths = project_points_to_image(points, cam_info_proj)  # 接收深度
        img = draw_projected_points(img, uv_pts, valid_pts, depths=depths, point_size_base=2)
        ...
        for i, box in enumerate(boxes_3d):
            ...
            # 绘制框（函数内部会过滤无效点）
            img = draw_projected_box(img, uv_box, valid_box, box_type=box['type'], thickness=2)
    ...
    # --- BEV 图 (NuScenes 风格) ---
    bev_out_path = out_dir / f"{base_name}_bev.jpg"
    visualize_bev_nuscenes_style(bev_out_path, points, boxes_3d, xlim=(-100,100), ylim=(-100,100))    
