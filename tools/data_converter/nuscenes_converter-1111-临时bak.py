...
def create_nuscenes_infos(root_path,
    ...
    if len(available_scenes) == 1:
        ...
        if test:
            ...
            # 训练验证模式：所有样本既用于训练也用于验证
            train_sample_tokens = set(all_sample_tokens)
            val_sample_tokens = set(all_sample_tokens)
            print(f"🔵[xmy修复]>>> 训练验证模式：{len(all_sample_tokens)}个样本同时用于训练和验证")
        ...
        available_vers = ['v1.0-trainval', 'v1.0-test', 'v1.0-mini', 
                         'v1.0-tyjt-trainval', 'v1.0-tyjt-test', 'v1.0-tyjt']
        
        if version in available_vers:
            # 标准版本使用官方划分
            if version == 'v1.0-trainval' or version == 'v1.0-tyjt-trainval':
                train_scenes = splits.train
                val_scenes = splits.val
                print(f"🔵[xmy修复]>>> 使用trainval划分")
            elif version == 'v1.0-test' or version == 'v1.0-tyjt-test':
                train_scenes = splits.test
                val_scenes = []
                print(f"🔵[xmy修复]>>> 使用test划分")
            elif version == 'v1.0-mini':
                train_scenes = splits.mini_train
                val_scenes = splits.mini_val
                print(f"🔵[xmy修复]>>> 使用mini划分")
        else:
            # 自定义版本使用自动场景划分
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        try:
            ...
            if is_sample_level:
                ...
                if not test and sample['token'] in val_scenes:  # 测试模式不生成验证集
                ...
                if sample['scene_token'] in train_scenes:
                    train_nusc_infos.append(info)
                    token2idx[info['token']] = ('train', len(train_nusc_infos) - 1)
                elif not test and sample['scene_token'] in val_scenes:  # 测试模式不生成验证集
                    val_nusc_infos.append(info)
                    token2idx[info['token']] = ('val', len(val_nusc_infos) - 1)
