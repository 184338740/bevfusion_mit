        ...
        try:
            ...
            if not test:
                ...
                for i in range(len(names)):
                    ...
                    # 如果是TYJT数据集，应用额外映射
                    if is_tyjt_dataset and mapped_name in TYJT_CATEGORY_MAPPING:
                        mapped_name = convert_tyjt_to_nuscenes_category(mapped_name)
                    
                    # 确保最终类别在nus_categories中
                    if mapped_name not in nus_categories:
                        # 如果不在标准类别中，映射到barrier
                        mapped_name = 'barrier'
                        if original_name != mapped_name:
                            print(f"🔵[xmy映射]>>> 类别转换: {original_name} -> {mapped_name}")
