...
def visualize_sample(sample_idx: int, info: Dict, out_dir: Path):
    ...
    # 从 lidar_path 中解析子包名（倒数第三级目录）
    sub_packet = Path(lidar_path).parent.parent.parent.name  # .../datasets/子包名/lidar/pcd/时间戳.npy
    base_name = f"{pkg_name}_{road_id}_{sub_packet}_{timestamp}"

    # 加载数据
    points, images, boxes_3d = load_sample(info)
    print(f"  样本 {sample_idx}: 标注数量 = {len(boxes_3d)}: base_name {base_name}: label_path {label_path}")
    ...
    for cam in CAM_NAMES:
        ...
        # 投影点云
        uv_pts, valid_pts = project_points_to_image(points, info['cams'][cam])
    ...
    # 点云范围过滤（仅显示感兴趣区域）
    # x_range = [-50, 50]   # 可根据实际场景调整
    # y_range = [-50, 50]
    x_range = [-100, 100]   # 扩大范围
    y_range = [-100, 100]
    ...
    # 绘制点云（极小点 + 高透明度）
    # ax.scatter(points_plot[:, 0], points_plot[:, 1], s=0.1, c='blue', alpha=1.0)
    ax.scatter(points_plot[:, 0], points_plot[:, 1],
            s=BEV_POINT_SIZE,
            c='blue',
            alpha=BEV_POINT_ALPHA)
    # 绘制 3D 框（俯视图）
    for box in boxes_3d:
        cx, cy, cz = box['center']
        l, w, h = box['size']
    ...
    ax.set_title(f'{base_name} BEV (annotations: {len(boxes_3d)}, points: {len(points_plot)}/{len(points)})', fontsize=14)
    ax.set_xlim(x_range)
    ax.set_ylim(y_range)
