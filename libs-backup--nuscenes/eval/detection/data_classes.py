...
def is_valid_attribute(attr_name):
    ...
    if attr_name in ATTRIBUTE_NAMES:
        return True
    # 🔵[TYJT修复]>>> 允许TYJT属性前缀
    if attr_name.startswith('tyjt.attribute.'):
        return True
    return False
...
class DetectionConfig:
    ...
    def __init__(self,
                 ...
                 mean_ap_weight: int):
        print(f"[xmy]>>> /opt/venv/lib/python3.8/site-packages/nuscenes/eval/detection/data_classes.py class_range.keys() = {class_range.keys()}; DETECTION_NAMES={DETECTION_NAMES} ")
...
class DetectionBox(EvalBox):
    ...
    def __init__(self,
        ...
        # 在DetectionBox的__init__方法中，放宽属性验证
        # assert attribute_name in ATTRIBUTE_NAMES or attribute_name == '', \
        #     'Error: Unknown attribute_name %s' % attribute_name
        assert is_valid_attribute(attribute_name), \
            'Error: Unknown attribute_name %s' % attribute_name
