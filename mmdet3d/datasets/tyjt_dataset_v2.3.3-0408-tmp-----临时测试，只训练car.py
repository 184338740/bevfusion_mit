...
class TYJTEval(DetectionEval):
    ...
    def __init__(self,
        ...
        if verbose:
            print(f">>>[xmy]🔵[TYJTDatasetV2]🔵[tyjt_dataset_v2.py] >>> ✅ TYJTEval 初始化完成: 预测框 {len(self.pred_boxes.all)} 个, "
    ...
    def _filter_boxes_by_range(self, eval_boxes: EvalBoxes, class_range: dict, verbose: bool) -> EvalBoxes:
        ...
        if verbose:
            print(f">>>[xmy]🔵[TYJTDatasetV2]🔵[tyjt_dataset_v2.py] >>> 过滤范围后: 框数从 {total_before} 减少到 {total_after}")
        return filtered
    ...
    def _load_tyjt_gt(self, infos: list, verbose: bool) -> EvalBoxes:
        ...
        for info in infos:
            ...
            for i, (box, name) in enumerate(zip(gt_boxes, gt_names)):
                ...
                if name not in DETECTION_NAMES:
                    if verbose:
                        print(f">>>[xmy]🔵[TYJTDatasetV2]🔵[tyjt_dataset_v2.py] >>> 警告: 未知类别 '{name}' (原始: {gt_names[i]})，跳过该框")
                    continue
        ...
        if verbose:
            print(f">>>[xmy]🔵[TYJTDatasetV2]🔵[tyjt_dataset_v2.py] >>> 加载 GT: {len(infos)} 个样本, {total_boxes} 个标注")
        return eval_boxes
