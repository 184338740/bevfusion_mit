...
class XmyCenterHeadPerCls(CenterHead):
    ...
    def get_task_detections(
        ...
        """Rotate nms for each task (支持 per-class NMS 阈值)."""
        predictions_dicts = []
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            if num_class_with_bg == 1:
                ...
                top_scores = cls_preds.view(-1)   # 替代 squeeze(-1)
                ...
                # # 对于多类，将二维得分矩阵转换为一维（每个框对应其预测类别的得分）
                # if top_scores.dim() == 2:
                #     top_scores = top_scores[torch.arange(top_scores.shape[0]), top_labels]
                    # top_scores = top_scores.gather(1, top_labels.unsqueeze(1)).squeeze(1)
            ...
            if Debug:
                print(f"[DEBUG] get_task_detections: task {task_id}, sample {i}:")
                print(f"  cls_preds shape: {cls_preds.shape}")
                print(f"  top_scores shape: {top_scores.shape}, top_labels shape: {top_labels.shape}")
            ...
            # 按类别分别执行 NMS（同样展平 mask）
            unique_labels = top_labels.unique()
            selected = []
            print(f"  Before NMS loop: top_scores shape={top_scores.shape}, top_labels shape={top_labels.shape}, unique_labels={top_labels.unique()}")
            ...
            if unique_labels.numel() > 0:
                for cls_label in unique_labels:
                    ...
                    if mask.dim() > 1:
                        mask = mask.view(-1)
                    if Debug:
                        print(f"    Class {cls_label.item()}: mask shape={mask.shape}, sum={mask.sum()}")
            ...
            if selected.numel() > 0:
                ...
                if Debug:
                    print(f"  After selection: selected_scores shape={selected_scores.shape}, selected_labels shape={selected_labels.shape}")
