...
/*
  Function: pillar pooling with sum and count (xmy version)
  Args:
    b                : batch size
    ...
    out_sum          : output sum features, FloatTensor[b, d, h, w, c]
    out_count        : output count per grid cell, FloatTensor[b, d, h, w]
*/
__global__ void bev_pool_sum_count_xmy_kernel(int b, int d, int h, int w, int n, int c, int n_intervals,
  ...
  float* cur_out_sum = out_sum + cur_geom_feats[3] * d * h * w * c + 
  ...
  *cur_out_sum = psum;
  // count: only write once per output cell (use first channel as representative)
  if (cur_c == 0) {
    float* cur_out_count = out_count + cur_geom_feats[3] * d * h * w + 
                           cur_geom_feats[2] * h * w + cur_geom_feats[0] * w + cur_geom_feats[1];
    *cur_out_count = interval_length;
  }
}

/*
  Function: pillar pooling backward (only for sum part) (xmy version)
  Args:
    b                : batch size
    d                : depth of the feature map
    h                : height of pooled feature map
...
*/
__global__ void bev_pool_sum_count_xmy_grad_kernel(int b, int d, int h, int w, int n, int c, int n_intervals,
...
// Launcher function (must match declaration in .cpp)
void bev_pool_sum_count_xmy(int b, int d, int h, int w, int n, int c, int n_intervals,
  ...
  bev_pool_sum_count_xmy_kernel<<<(int)ceil(((double)n_intervals * c / 256)), 256>>>(
    b, d, h, w, n, c, n_intervals, x, geom_feats, interval_starts, interval_lengths, out_sum, out_count
  );
}
...
void bev_pool_sum_count_xmy_grad(int b, int d, int h, int w, int n, int c, int n_intervals,
                                 ...
                                 const int* interval_starts, const int* interval_lengths,
                                 float* x_grad) {
  bev_pool_sum_count_xmy_grad_kernel<<<(int)ceil(((double)n_intervals * c / 256)), 256>>>(
